...
# File path
DATASET_FOLDER = './sudo/SA3-T01_Selected_Person_Characteristics_by_sex/'
...
def main():
    ...
    client.indices.create(index="person_age_sex_sa3", body=index_body, ignore=400)  # Ignore if index already exists
    
    success, failed = 0, 0
    for ok, response in streaming_bulk(
        client=client, index="person_age_sex_sa3", actions=generate_actions(),
    ):
        if not ok:
            failed += 1
            print(response)
