...
DATASET_FOLDER = './sudo/SA3-G02_Selected_Medians_and_Averages/'
...
def main():
    ...
    client.indices.create(index="age_personal_income_sa3", body=index_body, ignore=400)  # Ignore if index already exists
    
    success, failed = 0, 0
    for ok, response in streaming_bulk(
        client=client, index="age_personal_income_sa3", actions=generate_actions(),
    ):
        if not ok:
            failed += 1
            print(response)
